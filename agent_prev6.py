...
from livekit.agents.llm import ToolError  # (optional; available for raising tool errors)
...
# --- Vectorstore path resolution ---
VECTORSTORE_PATH = os.getenv("VECTORSTORE_PATH") or next(
    (
        p
        for p in (
            "/app/vectorstore",  # inside Docker/LiveKit Cloud
            "/Users/shamikalikhite/Documents/kitchenCompanion/vectorstore",  # local dev path
            os.path.join(os.getcwd(), "vectorstore"),  # repo-relative fallback
        )
        if os.path.exists(os.path.join(p, "index.faiss"))
        and os.path.exists(os.path.join(p, "index.pkl"))
    ),
    "/app/vectorstore",  # final fallback (will warn if files not present)
)
...
def initialize_rag() -> bool:
    ...
    if not (os.path.exists(faiss_idx) and os.path.exists(pkl_idx)):
        ...
        print("   To build it: python build_vectorstore.py <your_cookbook.pdf>")
        return False
